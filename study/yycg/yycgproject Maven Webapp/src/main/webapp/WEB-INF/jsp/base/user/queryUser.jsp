...
<script type="text/javascript">
	...
	var toolbar_v = [ {//工具栏
		...
		handler : function() {
			...
			createmodalwindow("添加用户信息", 800, 315, '${baseurl}user/addsysuser.action');
		}
	} ];
	...
	$(function() {
		$('#sysuserlist').datagrid({
			...
			striped : true,//条纹显示效果
			url : '${baseurl}user/queryUserResult.action',//加载数据的连接，连接请求过来是json数据
	...
	*/
	...
	function edituser(id){
		//alert(id);
		//打开修改窗口
		createmodalwindow("修改用户信息", 800, 315, '${baseurl}user/editsysuser.action?id='+id);
	}	
</script>
		...
		<TBODY>
			<TR>
				...
				<td><select name="sysuserCustom.userstate">
						<option value="">请选择</option>
 						<!-- 数据字典获取 -->
 						<c:forEach items="${userstateList}" var="p">
 							<option value="${p.dictcode}">${p.info}</option>
 						</c:forEach>
				</select></TD>				
