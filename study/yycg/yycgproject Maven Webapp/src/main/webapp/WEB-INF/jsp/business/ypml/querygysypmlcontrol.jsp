...
var columns = [ [{
	...
	hidden : true,
	formatter: function(value,row,index){
		return '<input type="hidden" name="gysypmlCustoms['+index+'].usergysid" value="'+row.usergysid+'" />';
	}
},{
	field : 'usergysmc',
	title : '供货企业',
...
function initGrid(){
	$('#gysypmllist').datagrid({
		...
		striped : true,
		//collapsible : true,
		url : '${baseurl}ypml/queryGysypmlControlResult.action',
		//sortName : 'code',
		//sortOrder : 'desc',
		//remoteSort : false,
					...
					<TR>
						...
						<td ><INPUT type="text" name="ypxxCustom.spmc" /></td>
						 <td class="left">价格范围：</td>
				  		<td>
				      		<INPUT id="ypxxCustom.zbjglower" name="ypxxCustom.priceStart" style="width:70px"/>
							至
							<INPUT id="ypxxCustom.zbjgupper" name="ypxxCustom.priceEnd" style="width:70px"/>
