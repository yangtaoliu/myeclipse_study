...
/*	function check_null(){
	...
	function check_null(){
		...
		{
			alert("登录名不能为空");
			$("input[name='logonName']")[0].focus();//转成dom对象才可以使用focus()
			return false;
		}
...
/* 	    if($.trim($("input[name='logonPwd']").val())=="")
		{
			alert("密码不能为空");
			$("input[name='logonPwd']")[0].focus();
			return false;
		} */		
    ...
    function findJctUnit(o){
    	//货物所属单位的文本内容
    	var jct = $(o).find("option:selected").text();//得到单位名称，用value()得到的是编号
    ...
    function checkUser(o){
    	...
    	$.post("elecUserAction_checkUser.do",{"logonName":logonName},function(data){
    		...
    		if(data==1){
				...
				$(o)[0].focus();//focus是dom对象，需要转成dom对象才能调用(加中括号，或者直接o.focus o是传过来的dom对象)
